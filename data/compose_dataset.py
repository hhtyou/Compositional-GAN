...
class ComposeAlignedDataset(BaseDataset):
    ...
    def initialize(self, opt):
        self.opt = opt
        with open(opt.datalist, newline='') as f:
            reader = csv.DictReader(f)#以csv形式读取
            self.info = [row['imgname'] for row in reader]
        ...
        if opt.random_view:
            ...
            #print(self.B_paths)
            self.mark_paths = [x.strip().split(' ')[1] for x in self.info]
            #print(self.mark_paths)
    ...
    def __getitem__(self, index):
        B_path = self.B_paths[index]
        #print(B_path)
        mark_path = self.mark_paths[index]
        #print(mark_path)
        if self.opt.random_view:
        ...
        landmark = pd.read_csv(mark_path,header=None)
        landmark_array = landmark.to_numpy()
        #print(landmark_array.shape)
        landmark = torch.tensor(landmark_array)
        ...
        out_dict = {'B': B, 'B_paths': B_path,'mark_paths': mark_path,'landmark': landmark,'B1':B1,'B2':B2,'B1_T':B1_T,'B2_T':B2_T, 'A1': A1, 'A2':A2, \
