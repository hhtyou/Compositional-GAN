...
from .landmark import landmarkLoader
from options.train_options import TrainOptions
import csv
from options.train_options import TrainOptions

from .vision_transforms import apply_transform_to_batch, vec_to_perpective_matrix, gen_random_perspective_transform
###############################################################################
# Functions
###############################################################################
...
def define_STN(input_nc, res=64, gpu_ids=[],y_x=1, STN_model=''):
    ...
    if STN_model =='deep':
        ...
        stn = HomographySpatialTransformer(input_nc, n_blocks, gpu_ids,y_x)
        #print("ZZZZZZZZZZZZZZZZZZZZZZZZZZzzzzzzzzzzzzzz",n_blocks)
...
class motoSpatialTransformer(nn.Module):
    def __init__(self, input_nc, n_blocks=0, gpu_ids=[],y_x=1):
        ...
        #self.xr = landmarkLoader(opt)

        # Regressor for the 3 * 2 affine matrix
        self.fc_loc = nn.Sequential(
            nn.Linear(30 * self.out_dim * self.out_dim +136, 32),
            nn.ReLU(True),
            nn.Linear(32, 3 * 4)
        )
    ...
    def forward(self, input, no_translatoin=False):
        ...
        xr = landmarkLoader(opt)*128/178
        #print("xr1", xr.shape)
        xr = torch.squeeze(xr, 1)
        #print("xr2", xr.shape)
        #xr = torch.LongTensor(xr).cpu()
        xr = xr.to(dtype=torch.float, device=torch.device("cpu"))
        #print("xr3", xr.shape)
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, input):
        ...
        input_1 = apply_transform_to_batch(input_1, P_init)
        input_2 = apply_transform_to_batch(input_2, P_init)

        theta_1 = nn.parallel.data_parallel(self.fc_loc, input_1, self.gpu_ids)
        theta_2 = nn.parallel.data_parallel(self.fc_loc, input_2, self.gpu_ids)
...
################################################################
class HomographySpatialTransformer(nn.Module):
    def __init__(self, input_nc, n_blocks=0, gpu_ids=[],y_x=1):
        super(HomographySpatialTransformer, self).__init__()
        self.gpu_ids = gpu_ids
        self.input_nc = input_nc
        self.n_blocks = n_blocks
        # Spatial transformer localization-network
        ...
        self.fc_loc[2].weight.data.fill_(0)
        self.fc_loc[2].bias.data = torch.FloatTensor([1, 0, 0, 0, 1, 0, 1, 0, 0, 0, 1, 0, 1, 0, 0, 0, 1, 0])
        #self.xr = landmarkLoader(opt)
    def forward(self, input, no_translatoin=False):
        from unittest.mock import Mock
        ...
        input = torch.mul(input,mask) + (Variable(ONES) - mask)
        if len(self.gpu_ids)  and isinstance(input.data, torch.cuda.FloatTensor):
            xs1 = nn.parallel.data_parallel(self.localization, input, self.gpu_ids)
        else:
            xs = self.localization(input)
      
        print("--------------------xs------------------------", xs1.shape)  
    
        xs = xs1.view(-1, 30 * self.out_dim * self.out_dim)
        print("--------------------xs------------------------", xs.shape)

        aind1 = Variable(LongTensor(range(0,2)))
        aind2 = Variable(LongTensor(range(2,4)))
        ainp1 = Variable(LongTensor(range(0,int(input.size(1)/2))))
        ainp2 = Variable(LongTensor(range(int(input.size(1)/2),input.size(1)))) 
        ind1 = Variable(LongTensor(range(0,8)))
        ind2 = Variable(LongTensor(range(8,16)))
        inp1 = Variable(LongTensor(range(0,int(input.size(1)/2))))
        ...
        #P_net=vec_to_perpective_matrix(theta)

        theta_1 = index_select(theta,1, ind1) #[10,2]
        print("--------------------theta_1------------------------", theta_1.shape)
        theta_2 = index_select(theta,1, ind2)
        P_net_1=vec_to_perpective_matrix(theta_1)#[10,3,3]
        print("--------------------P_net_1------------------------", P_net_1.shape)
        P_net_2=vec_to_perpective_matrix(theta_2)
        ...
        input_1 = apply_transform_to_batch(xs1, P_net_1)
        input_2 = apply_transform_to_batch(xs1, P_net_2)

        #input_1 = index_select(input, 1, inp1)
        print("--------------------input_1------------------------", input_1.shape)
        #input_2 = index_select(input, 1, inp2)

        xx1 = apply_transform_to_batch(input_1, P_net_1)
        xx2 = apply_transform_to_batch(input_2, P_net_2)
        #x1 = nn.parallel.data_parallel(self.fc_loc,input_1, self.gpu_ids)
        #x2 = nn.parallel.data_parallel(self.fc_loc,input_2, self.gpu_ids)
...
class motoHomographySpatialTransformer(nn.Module):
    def __init__(self, input_nc, n_blocks=0, gpu_ids=[],y_x=1):
        super(HomographySpatialTransformer, self).__init__()
        self.gpu_ids = gpu_ids
        self.input_nc = input_nc
...
##############################################################
        if len(self.gpu_ids)  and isinstance(input.data, torch.cuda.FloatTensor):        
            p1 = nn.parallel.data_parallel(self.fc_loc, xs, self.gpu_ids)
            p2 = nn.parallel.data_parallel(self.fc_loc, xs, self.gpu_ids)
            #net = net.view(-1,4,3)
            print("--------------------p2------------------------", p2.shape)
            P1_net = vec_to_perpective_matrix(p1).cuda()
            P2_net = vec_to_perpective_matrix(p2).cuda()
            print("--------------------P1_net------------------------", P1_net.shape)
...
#2
        ...
        x1 = apply_transform_to_batch(input_1,P1_net)
        x2 = apply_transform_to_batch(input_2,P2_net)
...
class SpatialTransformerdeep(nn.Module):
    def __init__(self, input_nc, n_blocks=0, gpu_ids=[],y_x=1):
        super(SpatialTransformerdeep, self).__init__()
        self.gpu_ids = gpu_ids
        self.input_nc = input_nc
        self.n_blocks = n_blocks
        # Spatial transformer localization-network
        ...
        # Regressor for the 3 * 2 affine matrix
        self.fc_loc = nn.Sequential(
            nn.Linear(128 * self.out_dim * self.out_dim+136, 128),
            nn.ReLU(True),
            nn.Linear(128, 64),
            nn.ReLU(True),
            nn.Linear(64, 3*4)
